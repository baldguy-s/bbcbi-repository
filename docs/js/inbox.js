...
export async function renderInboxView(container) {
  ...
  container.innerHTML = `
    ...
    <div id="inbox-upload"></div>
    <div id="inbox-list"></div>
  `;

  attachUploadWidget(container.querySelector('#inbox-upload'), { inbox: 'true' }, {
    onUploaded: (newFiles) => {
      items.unshift(...newFiles);
      renderList();
    },
  });
  ...
  // Attached once to the (stable) container, not inside renderList() — that
  // function re-runs on every upload, and a fresh listener stacking on each
  // call would fire an action once per accumulated listener.
  listEl.addEventListener('click', async (ev) => {
    ...
    if (!isOpen) await buildFilingPanel(panel, fileId, years, () => {
      const idx = items.findIndex((i) => String(i.id) === fileId);
