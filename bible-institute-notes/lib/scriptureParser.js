// Regex-based KJV scripture reference extraction.
//
// Only fires on "Book<space>chapter" (a book alias immediately followed by a
// number) — a bare book mention with no chapter number is never captured,
// since several book names/abbreviations (Job, Acts, Judges, Titus, Mark...)
// are also ordinary English words or names, and false-positiving on those
// would make the scripture index noisy and untrustworthy.

// Canonical order doubles as biblical book order for the Scripture Index view.
const BOOKS = [
...
// Longest-alias-first so e.g. "1 corinthians" matches before "1 co".
...
// Aliases already contain a literal space for multi-word/numbered forms
// (e.g. "1 corinthians"); allow that space to be one-or-more whitespace so
// "1  Corinthians" or "1\nCorinthians" still match.
...
function extractRefs(bodyMarkdown) {
  ...
  while ((match = REF_REGEX.exec(bodyMarkdown)) !== null) {
    ...
    if (verseStartStr && rangeEndStr) {
      if (verseEndAfterColonStr) {
        // Cross-chapter range, e.g. "8:28-9:5" -> rangeEndStr=9 (end chapter), verseEndAfterColonStr=5 (end verse)
        chapterEnd = parseInt(rangeEndStr, 10);
        verseEnd = parseInt(verseEndAfterColonStr, 10);
      } else {
        // Same-chapter range, e.g. "8:28-30"
...
function bookSortIndex(book) {
  return BOOK_ORDER.has(book) ? BOOK_ORDER.get(book) : BOOKS.length;
}

module.exports = { extractRefs, BOOKS, bookSortIndex };
